%% Script for CADASIL paper & Thesis - Feb 2017 & now Oct 2017 (although ~cut and pasted from previous scripts)
%% Intention is for this to be the 'final' script for CADASIL analysis, generating all figures required plus
...
... 2) Running of competing computational models - in this case includes the 
... GLME (which produced somewhat counterintuitive results), the straight
... logistic regression (which produced somewaht counterintuitive results)
... and possibly the exponential model (or others for comparison e.g. quadratic)

... 3) Analysis and comparison of model parameters + eg pupil data (+/- imaging)
...
% I want to look at log values. 
close all
figure()
%bar(1,nanmean(dtYes_HC(:,1)),'FaceColor',[0.7 0.7 0.7]);hold on;errorbar(1,nanmean(dtYes_HC(:,1)),nanstd(dtYes_HC(:,1))./sqrt(19),'k','LineWidth',3);
%bar(2,nanmean(dtYes_HC(:,2)),'FaceColor',[0.3 0.3 0.3]);hold on;errorbar(2,nanmean(dtYes_HC(:,2)),nanstd(dtYes_HC(:,2))./sqrt(19),'k','LineWidth',3)
h1 = bar(1,nanmean(dtYesT(ap==0,1)),'FaceColor',[0.7 0.7 0.7]);hold on;errorbar(1,nanmean(dtYesT(ap==0,1)),nanstd(dtYesT(ap==0,1))./sqrt(length(find(ap==0))),'k','LineWidth',3); 
h2 = bar(2,nanmean(dtYesT(ap==0,2)),'FaceColor',[0.3 0.3 0.3]);hold on;errorbar(2,nanmean(dtYesT(ap==0,2)),nanstd(dtYesT(ap==0,2))./sqrt(length(find(ap==1))),'k','LineWidth',3); 
h3 = bar(4,nanmean(dtYesT(ap==1,1)),'FaceColor',[0.7 0.7 0.7]);hold on;errorbar(4,nanmean(dtYesT(ap==1,1)),nanstd(dtYesT(ap==1,1))./sqrt(length(find(ap==0))),'k','LineWidth',3);
h4 = bar(5,nanmean(dtYesT(ap==1,2)),'FaceColor',[0.3 0.3 0.3]);hold on;errorbar(5,nanmean(dtYesT(ap==1,2)),nanstd(dtYesT(ap==1,2))./sqrt(length(find(ap==1))),'k','LineWidth',3);
ax=gca;
set(ax,'fontWeight','bold','fontSize',20,'XTick',[1.5 4.5],'XTickLabel',{})
...
xticklabels({'NoAP','Ap'});
legend([h1 h2],{'Yes','No'});
title('(log) Decision time vs Trial Type')
...
% I want to look at choice without apathy. 
close all
figure()
%bar(1,nanmean(dtYes_HC(:,1)),'FaceColor',[0.7 0.7 0.7]);hold on;errorbar(1,nanmean(dtYes_HC(:,1)),nanstd(dtYes_HC(:,1))./sqrt(19),'k','LineWidth',3);
%bar(2,nanmean(dtYes_HC(:,2)),'FaceColor',[0.3 0.3 0.3]);hold on;errorbar(2,nanmean(dtYes_HC(:,2)),nanstd(dtYes_HC(:,2))./sqrt(19),'k','LineWidth',3)
h1 = bar(1,nanmean(dtYesT(:,1)),'FaceColor',[0.7 0.7 0.7]);hold on;errorbar(1,nanmean(dtYesT(:,1)),nanstd(dtYesT(ap==0,1))./sqrt(55),'k','LineWidth',3); 
h2 = bar(2,nanmean(dtYesT(:,2)),'FaceColor',[0.3 0.3 0.3]);hold on;errorbar(2,nanmean(dtYesT(:,2)),nanstd(dtYesT(ap==0,2))./sqrt(55),'k','LineWidth',3); 

ax=gca;
set(ax,'fontWeight','bold','fontSize',20,'XTick',[1 2],'XTickLabel',{})
ylabel('(log)Decision time (s)');
xlim([0 3]);
%ylim([-5 5]);
xticklabels({'Yes','No'});
...
% do the same for the Log Values
figure()
temp=dtYesT(:,2)-dtYesT(:,1);%tempHC=dtYes_HC(:,2)-dtYes_HC(:,1);
%bar(1,nanmean(tempHC),'m');hold on;errorbar(1,nanmean(tempHC),nanstd(tempHC)./sqrt(19),'k','LineWidth',3);
bar(1,nanmean(temp(ap==0)),'Facecolor',color1);hold on;errorbar(1,nanmean(temp(ap==0)),nanstd(temp(ap==0))./sqrt(length(find(ap==0))),'k','LineWidth',3);
bar(2,nanmean(temp(ap==1)),'Facecolor',color2);hold on;errorbar(2,nanmean(temp(ap==1)),nanstd(temp(ap==1))./sqrt(length(find(ap==1))),'k','LineWidth',3);
if 1
    ...
    plot(vecA,nanmean(temp(ap==1,:),2),'.','MarkerSize',12,'MarkerEdgeColor',[0.5 0.5 0.5])
    if 1
        plot(3,-.93,'.','MarkerSize',12,'MarkerEdgeColor',[0.5 0.5 0.5]);
    end
end
ax=gca;
set(ax,'fontWeight','bold','fontSize',20,'XTick',[1 2],'XTickLabel',{'noAp','Ap'})
...
easyDec_transform = nanzscore(log(easyDec));
easyDec_transform(19,:) = []; 
figure()
%errorBarPlot(easyDecHC,'Color',[0.7 0.7 0.7],'LineWidth',3);hold on
errorBarPlot(easyDec_transform(ap==0,:),'Color',color1,'LineWidth',3);
hold on 
errorBarPlot(easyDec_transform(ap==1,:),'Color',color2,'LineWidth',3);
legend('SVD No Apathy','SVD Apathy')
ax=gca;
set(ax,'fontWeight','bold','fontSize',20,'XTick',[1 2],'XTickLabel',{'Easy','Hard'})
ylabel('Decision time (s)');
...
'choice ~ rew + (rew|subject)'
'choice ~ eff + (eff|subject)'
'choice ~ 1 + (1|subject)'
