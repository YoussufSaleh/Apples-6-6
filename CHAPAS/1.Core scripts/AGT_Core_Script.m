...
%%% SCRIPT structure: 
%1. Questionnaire correlations: to establish a correlation matrix
%2. Behavioural Plots: Data visualisation for Choice and DT with
%regressors derived from the questionnaire data
%3. Statistical analyses:  Setting up the Generalised linear mixed
% effects model to run an analysis for the behavioural data (choice data,
%4. Computational modelling: Generate computational model variables and
%plot outcomes of computational model that best fits the data. 

clear
close all
load AGT_grpD_CHAPAS25.mat % This should be generated by the last line of the script ...
...
load Qs % these are variable names I have created to ...
...
% protocol). 
AMI.Total=Qs{:,6}/18;
AMI.Behaviour=Qs{:,5}/6;
AMI.emotional=Qs{:,4}/6;
AMI.social=Qs{:,3}/6;


apVec=[]';
for i = 1:subj
  if AMI.Behaviour(i) > 2.34
...
if 0
    figure()
    h = scatterRegress(AMI.Total,aesVec,'*','MarkerEdgeColor',[0.5 0.5 0.5],'LineWidth',5);
    h = ax;
    set(gca,'LineWidth',2)
    xlim([-36 11])
...
if 0 % draw some other graphs of correlations.
    figure()
    scatterRegress(AMI.Total,accept);
    xlabel('Increasing apathy severity --> (AMI total)')
    ylabel('Number of offers accepted');
    title('AMI & Offers accepted');
    hold off
    figure()
    scatterRegress(AMI.Behaviour,accept);hold on;
    xlabel('Increasing severity on LARS Action Initiation subscale')
    ylabel('Number of offers accepted');
    title('AMI behavioural & Offers accepted');
    figure()
    scatterRegress(AMI.emotional,accept);hold on;
    xlabel('Increasing apathy severity --> LARS ')
    ylabel('Number of offers accepted');
    title('AMI emotional & Offers accepted');
...
xlim([0.5 2.5])
ax=gca;
set(ax,'XTick',[1 2],'fontWeight','bold','fontSize',20,'ylim',[0.3 1],'YTick',[.3:.1:1],'YTickLabel',{'0.5','0.6','0.7','0.80','0.9','1.0'},'XTicklabel',{'No Apathy','Apathy'});
